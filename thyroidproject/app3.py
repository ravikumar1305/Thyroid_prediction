...
def xgboost(data_new):
    clean1 = pd.DataFrame(mode_impute.transform(data_new), columns = data_new.select_dtypes(include = ['object']).columns)
    clean2 = pd.DataFrame(median_impute.transform(data_new), columns = data_new.select_dtypes(exclude = ['object']).columns)

    clean3 = pd.DataFrame(encoding.transform(clean1))
    clean4 = pd.DataFrame(minmax.transform(clean2))

    clean_data = pd.concat([clean4, clean3], axis = 1, ignore_index = True)
    prediction = pd.DataFrame(model.predict(clean_data), columns = ['Predictions'])
